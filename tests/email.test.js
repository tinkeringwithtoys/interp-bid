...
test('maps complete SMTP configuration', () => { const config = smtpConfigFromEnv({ SMTP_HOST: 'smtp.example.com', SMTP_PORT: '587', SMTP_USERNAME: 'user', SMTP_PASSWORD: 'pass', SMTP_FROM: 'scout@example.com', REPORT_RECIPIENTS: 'team@example.com' }); assert.equal(config.secure, false); assert.equal(config.requireTls, true); assert.deepEqual(config.recipients, ['team@example.com']); });
