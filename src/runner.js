...
async function runLanguage({ language, settings, policy, registry, now, provider }) {
  ...
  if (provider === 'agnes-exa') {
    ...
    const batches = await Promise.all(queries.map((query) => exaSearch(query, process.env.EXA_API_KEY))); const byUrl = new Map();
...
async function main() {
  ...
  const registry = await readJson('state/lead-registry.json'); const provider = process.env.RESEARCH_PROVIDER || settings.defaultProvider, now = new Date(), results = [];
  ...
  const reportDir = path.join(root, 'reports', day(now)); await fs.mkdir(reportDir, { recursive: true }); const reports = [];
  for (const result of results) { const content = renderReport({ date: day(now), language: result.language, digest: result.digest, delivered: result.delivered, suppressed: result.suppressed }); reports.push({ language: result.language, content }); await fs.writeFile(path.join(reportDir, `${result.language}-${clock(now)}.md`), content); }
  const newLeadCount = results.reduce((sum, item) => sum + item.delivered.length, 0);
  if (newLeadCount > 0) {
    const smtp = smtpConfigFromEnv();
    if (!smtp) throw new Error('New leads were found but SMTP is not configured. Add SMTP_HOST, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM, and REPORT_RECIPIENTS as Actions secrets.');
    await sendSmtpMail({ config: smtp, subject: `interp-bid: ${newLeadCount} new or updated interpreting lead${newLeadCount === 1 ? '' : 's'} — ${day(now)}`, text: reports.map((report) => report.content).join('\n\n' + '='.repeat(72) + '\n\n') });
  }
