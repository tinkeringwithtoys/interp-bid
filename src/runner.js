...
async function runLanguage({ language, settings, policy, registry, now, provider }) {
  ...
  return { digest, delivered, suppressed, diagnostics: { queries, candidateCount: candidates.length, modelShortlistCount: digest.shortlist.length, manualVerificationCount: digest.manualVerification.length, sourceUrlCount: response.sourceUrls?.length || 0, candidates: candidates.slice(0, 10).map((item) => ({ title: item.title, url: item.url })) } };
}
async function main() {
  const settings = await readJson('config/settings.json'); const policy = (await fs.readFile(path.join(root, 'config/scout-prompt.md'), 'utf8')).replaceAll('[Company]', settings.companyName); const registry = await readJson('state/lead-registry.json'); const provider = process.env.RESEARCH_PROVIDER || settings.defaultProvider, now = new Date(), results = [];
  ...
  if (process.env.DRY_RUN === 'true') { console.log(JSON.stringify({ dryRun: true, provider, results: results.map((item) => ({ language: item.language, newLeads: item.delivered.length, suppressed: item.suppressed, diagnostics: item.diagnostics })) }, null, 2)); return; }
  ...
  if (newLeadCount > 0) { const smtp = smtpConfigFromEnv(); if (!smtp) throw new Error('New leads were found but SMTP is not configured. Add SMTP_HOST, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM, and ALERT_EMAIL as Actions secrets.'); await sendSmtpMail({ config: smtp, subject: `interp-bid: ${newLeadCount} new or updated interpreting lead${newLeadCount === 1 ? '' : 's'} — ${day(now)}`, text: reports.map((report) => report.content).join('\n\n' + '='.repeat(72) + '\n\n') }); }
